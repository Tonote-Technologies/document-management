	...
	foreach($documentResource as  $savedTool){
		...
		if($savedTool->tool_type == 1){
			if ($savedTool->tool_name == "Textarea") {
				...
				// 						<textarea oninput="this.parentNode.dataset.value = this.value" rows="1" class="textareaTool '.$visible.'" value="'.$text_value.'" data-id="'.$savedTool->tool_id.'">'.$text_value.'</textarea>
				// 					</div>
				// 		    </dl>';
				$output .= '
							...
							';
				// $output .=' <label class="input-sizer">
				// 				<span>Name: </span>
